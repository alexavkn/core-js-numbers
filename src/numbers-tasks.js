...
function roundToPowerOfTen(num, pow) {
  ...
  const numStrLength = numStr.split('').length;
  numStr = '0.'.concat(numStr);
  let newNumStr = Number(numStr)
    .toFixed(numStrLength - pow)
    .split('.')[1];
  if (newNumStr === '0') {
    newNumStr = '1';
    for (let i = 0; i < numStrLength; i += 1) {
      newNumStr = newNumStr.concat('0');
    }
    return Number(newNumStr);
  }
  const newNumStrLength = newNumStr.length;
  for (let i = 0; i < numStrLength - newNumStrLength; i += 1) {
    newNumStr = newNumStr.concat('0');
  }
  return Number(newNumStr);
}
